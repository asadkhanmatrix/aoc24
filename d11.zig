...
fn forN(allocator: std.mem.Allocator, n: u128, s: u128, mp: *std.StringHashMap(u128), all_keys: *std.ArrayList([]const u8)) u128 {
    if (s == 0) unreachable;
    if (s == 1) {
    ...
    all_keys.append(key) catch unreachable;
    if (mp.contains(key)) {
        return mp.get(key).?;
    }
    if (n == 0) {
        const res = forN(allocator, 1, s - 1, mp, all_keys);
        mp.put(key, res) catch unreachable;
        return res;
    }
    if (@mod(digitCount(n), 2) == 1) {
        const res = forN(allocator, n * 2024, s - 1, mp, all_keys);
        mp.put(key, res) catch unreachable;
        return res;
    }
    ...
    const res = forN(allocator, n1, s - 1, mp, all_keys) + forN(allocator, n2, s - 1, mp, all_keys);
    mp.put(key, res) catch unreachable;
    return res;
}
...
pub fn main() !void {
    ...
    var all_keys = std.ArrayList([]const u8).init(allocator);
    defer {
        for (all_keys.items) |s| {
            allocator.free(s);
        }
        all_keys.deinit();
    }
    ...
    var res: u128 = 0;
    for (l.items) |v| {
        res += forN(allocator, v, steps1, &mp, &all_keys);
    }
    std.debug.print("part 1: {}\n", .{res});
    res = 0;
    for (l.items) |v| {
        res += forN(allocator, v, steps2, &mp, &all_keys);
    }
